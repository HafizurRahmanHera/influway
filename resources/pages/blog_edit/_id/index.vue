                ...
                <div class="_1blog_section_all _box_shadow _border_radious _mar_b20 _padd25">
                  ...
                  <div class="row">
                    <div class="col-12 col-md-12">
                      <h3 class="blog_section_left_h3">Edit Blog</h3>
                    </div>
                    ...
                    <div class="col-12 col-md-12 col-lg-12">
                      <div class="_input_button">
                        <button :disabled="loading" @click="updateBlog" class="btn btn-primary lift" type="button">Update</button>
...
export default {
  ...
  async asyncData({ app, store, redirect, params }) {
    try {
      let [res1, res2] = await Promise.all([
        ...
        app.$axios.get(`/getUserBlog/${params.id}`)
      ]);
      if (!res2.data) {
        // redirect("/blog");
        redirect("/");
  ...
  created() {
    ...
    this.quill_char_length=this.blog.text.length
    if (this.blog.category_id) {
      this.form.category_id = this.blog.category_id;
    }
    this.form.text = this.blog.text;
    this.form.feature_image = this.blog.feature_image;
    if (this.blog.feature_image) {
      let img = {
        name: "image",
        percentage: 100,
        status: "finished",
        // uid:1234567,
        url: this.blog.feature_image
      };
      this.uploadList.push(img);
    }
    this.form.tags = this.blog.blogTags;
  },
  ...
  methods: {
    ...
    async updateBlog() {
      ...
      // return
      const res = await this.callApi("post", "/updateBlog", this.form);
      if (res.status === 200) {
        // console.log(res);
        this.s("Blog updated Successfully !");
    ...
    async handleRemove(file, index) {
      ...
      // console.log(file)
      if (this.blog.feature_image == file.url) {
