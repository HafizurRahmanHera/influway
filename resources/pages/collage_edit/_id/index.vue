      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-12 col-md-12 col-lg-12">
            <div class="card _border_radious _mar_b20 _padd15">
              <h3 class="creat_section_h3">Edit Collage</h3>
              <div class="row">
                <div class="col-md-12">
                ...
                <div class="col-12 col-md-12 col-lg-auto">
                  ...
                  <div class="_input_button _btn_pro">
                    <template v-if="is_remove_linked_pro">
                      <button class="btn btn-default lift" @click="cancelRemove">Cancel</button>
                    </template>

                    <template v-else>
                      <button class="btn btn-primary lift" @click="removeImage" type="button">Remove image</button>
                      <button :disabled="loading" class="btn btn-primary lift" @click="updateCollageLinks" type="button">Update</button>
...
export default {
  ...
  async asyncData({ app, store, redirect, params }) {
    try {
      let [res1, res2] = await Promise.all([
        ...
        app.$axios.get(`/getUserCollageProduct/${params.id}`)
      ]);
      if (!res2.data) {
        redirect("/collage");
      }
  ...
  created() {
    this.collage_product.linkedProducts.forEach(items => {
      // console.log(items);
      let item = {
        x: items.x,
        y: items.y,
        ...
        h: items.h,
        i: items.i,
        image: items.product.productImages[0].image
      };
  ...
  data() {
    return {
      loading: false,
      layout: [
        { x: 0, y: 0, w: 2, h: 6, i: "1", image: "/products_image/cote.jpg" },
        { x: 0, y: 12, w: 2, h: 6, i: "2", image: "/products_image/cote2.jpg" },
        { x: 2, y: 0, w: 4, h: 18, i: "3", image: "/products_image/cote3.jpg" },
        { x: 0, y: 6, w: 2, h: 6, i: "4", image: "/products_image/cote5.jpg" },
        { x: 0, y: 6, w: 2, h: 6, i: "5", image: "/products_image/cote4.jpg" }
      ],
      search: "",
      linked_pro: [],
  ...
  methods: {
    ...
    async updateCollageLinks() {
      ...
      // return
      const res = await this.callApi("post", "/updateCollageLinks", {
        collage_product_links: this.linked_pro,
        collage_product_id: this.$route.params.id
      });
      // console.log(res);
      if (res.status === 200) {
        // this.products = res.data
        // console.log(res);
        this.s("Collage product link updated");
